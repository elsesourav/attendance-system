...
export default function ManageStreamStudents() {
  ...
  useEffect(() => {
    const fetchStreamAndStudents = async () => {
      try {
        ...
        const streamResponse = await fetch(`/api/teacher/streams/${streamId}`);
        ...
        const enrolledResponse = await fetch(`/api/teacher/streams/${streamId}/students`);
        ...
        const availableResponse = await fetch(`/api/teacher/students/available?streamId=${streamId}`);
  ...
  const handleEnrollStudent = async (studentId: number) => {
    try {
      const response = await fetch(`/api/teacher/streams/${streamId}/students`, {
        method: "POST",
        headers: {
  ...
  const handleUnenrollStudent = async (studentId: number) => {
    if (!confirm("Are you sure you want to remove this student from the stream?")) {
      return;
    }

    try {
      const response = await fetch(`/api/teacher/streams/${streamId}/students/${studentId}`, {
        method: "DELETE",
      });
          ...
          <div className="space-y-6">
            ...
            {enrolledStudents.length === 0 ? (
              <div className="text-center py-6 bg-muted/20 rounded-lg">
                <p className="text-muted-foreground">No students enrolled in this stream yet</p>
                <Button 
                  variant="outline" 
                  className="mt-2"
